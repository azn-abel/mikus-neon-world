  ...
  player.addListener({
    ...
    onTimerReady() {
      ...
      if (window.textureLoaded) {
        ...
        audioElement.volume = (document.getElementById("myRange").value)/100;
        if (currentLanguage === "en") {
    ...
    onTimeUpdate(position) {
      ...
      if (!current.next || current.next.startTime - current.startTime > 1500) {
        ...
        if (!current.next) {
          document.getElementById("lyrics").style.opacity = document.getElementById("lyrics").style.opacity - .025;
        }
        else if (restCount >= 4) {
